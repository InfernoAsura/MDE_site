...
def sgm(left_path, right_path, left_gt_path=None, right_gt_path=None,
        output_path="disparity_map.png", max_disp=64, show_images=False, eval_mode=False):
    import time as t
    import os
    ...
    print('\nLoading images...')
    left, right = load_images(left_path, right_path, parameters)

    print('\nStarting cost computation...')
    left_cost_volume, right_cost_volume = compute_costs(left, right, parameters, show_images)
    if show_images:
        left_disp_map_dbg = np.uint8(normalize(np.argmin(left_cost_volume, axis=2), parameters))
        right_disp_map_dbg = np.uint8(normalize(np.argmin(right_cost_volume, axis=2), parameters))
        cv2.imwrite('disp_map_left_cost_volume.png', left_disp_map_dbg)
        cv2.imwrite('disp_map_right_cost_volume.png', right_disp_map_dbg)

    print('\nStarting left aggregation computation...')
    left_agg_volume = aggregate_costs(left_cost_volume, parameters, paths)
    print('\nStarting right aggregation computation...')
    right_agg_volume = aggregate_costs(right_cost_volume, parameters, paths)

    print('\nSelecting best disparities...')
    left_disp_map = np.uint8(normalize(select_disparity(left_agg_volume), parameters))
    right_disp_map = np.uint8(normalize(select_disparity(right_agg_volume), parameters))

    print('\nApplying median filter...')
    left_disp_map = cv2.medianBlur(left_disp_map, parameters.bsize[0])
    right_disp_map = cv2.medianBlur(right_disp_map, parameters.bsize[0])

    basename = os.path.basename(output_path)
    dirname = os.path.dirname(output_path)
    left_out = os.path.join(dirname, f"left_{basename}")
    right_out = os.path.join(dirname, f"right_{basename}")

    cv2.imwrite(left_out, left_disp_map)
    cv2.imwrite(right_out, right_disp_map)

    recall_output = []
    if eval_mode and left_gt_path and right_gt_path:
        print('\nEvaluating left disparity map...')
        recall_l = get_recall(left_disp_map, left_gt_path, type('Args', (object,), {'disp': max_disp}))
        print('\tLeft Recall = {:.2f}%'.format(recall_l * 100.0))
        recall_output.append(f"Left Recall = {recall_l * 100:.2f}%")

        print('\nEvaluating right disparity map...')
        recall_r = get_recall(right_disp_map, right_gt_path, type('Args', (object,), {'disp': max_disp}))
        print('\tRight Recall = {:.2f}%'.format(recall_r * 100.0))
        recall_output.append(f"Right Recall = {recall_r * 100:.2f}%")

    dusk = t.time()
    print('\nFin.')
    print('\nTotal execution time = {:.2f}s'.format(dusk - dawn))

    return left_out, right_out, recall_output
